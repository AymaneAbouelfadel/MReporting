...
import CardCumul from '../Card/CardVCumul.js'

export function VCumul () {
  const [venteCumulData, setCumulData] = useState([]); 
  // CALENDRIER
 ...
 useEffect(() => {
   ...
   const toggleCalendar = () => {
     ...
     //setShowAfficherButton(false); // Une fois que vous avez cliqué sur le bouton "Calendrier", masquez le bouton "Afficher"
   };
   ...
   const onClick = () => {
     ...
     let apiUrl = 'http://192.168.56.1:3003/cumul?';
     ...
     if (selectedSiteLabel !== null) {
       queryParams.push(`site=${selectedSiteLabel}`);
     }
     ...
     if (selectedModeleLabel !== null) {
       queryParams.push(`modele=${selectedModeleLabel}`);
     }
   ...
   return(
     ...
     <ScrollView>
       ...
       <TouchableOpacity style={styles.buttonCalendrier} onPress={toggleCalendar}>
         ...
         {showCalendar && (
           <View style={styles.calendarContainer}>
             <Calendar
               ...
               onDayPress={
                (day) => handleDateSelect(day.dateString)}
         ...
         {showCard && (
           <CardCumul content={venteCumulData} />
         )}
         </ScrollView>
     </ImageBackground>
 );
